...
----------------------------
	...
	Compile source code and build JAR file:
    > javac -classpath "lib/*"  -d pr *.java 
    > jar -cvf pr.jar -C pr .

    Transfer JAR file to Hadoop server:
    > scp -P 2222 pr.jar <userid>@infpro52.reutlingen-university.de:/home/<userid>

    
Steps on Hadoop Server
----------------------

	Examine input:
	$ hadoop fs -cat /user/cbdt00/wikipedia/input/wikipedia-articles.xml
    
    Execute Map/Reduce application: 
    $ hadoop jar pr.jar WikiPageRanking

	Examine result:
    $ hadoop fs -cat wikipedia/result/part-r-00000

	Cleanup:
    $ hadoop fs -rm -r wikipedia
...
public class WikiPageRanking extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        boolean isCompleted = runXmlParsing("/user/cbdt00/wikipedia/input/wikipedia-articles.xml", "wikipedia/ranking/iter00");
        if (!isCompleted) return 1;
        String lastResultPath = null;

        for (int runs = 0; runs < 5; runs++) {
            String inPath = "wikipedia/ranking/iter" + nf.format(runs);
            lastResultPath = "wikipedia/ranking/iter" + nf.format(runs + 1);
        ...
        isCompleted = runRankOrdering(lastResultPath, "wikipedia/result");
